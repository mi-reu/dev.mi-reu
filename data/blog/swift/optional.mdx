...
근데 만약 강제로 언래핑을 했는데 값이 `nil` 이면 어떻게 될까요? 예상하시는 것처럼 오류가 발생하여 앱이 죽는 형상이 발생할 수 있습니다.
...
이렇게 하게되면 출력 결과는 `nil` 이 호출되게 될겁니다. 당연하게도 member 저장속성에는 아직 `Member` 의 인스턴스가 담겨있지 않기 때문이죠.

```swift
family.member = Member(name: "밀러", age: 20)
...
실제 값을 얻기 위해서는 옵셔널 바인딩과 같은 추가로 포장지를 벗겨내는 과정이 더 필요할 뿐이죠.

추가로 예제에서는 저장 속성을 사용하여 예시를 보여줬지만 메서드도 저장 속성과 동일한 과정을 통해 값을 얻을 수 있습니다.
궁금하신 분은 위 코드를 복사해서 직접 출력 결과를 확인해보세요!
